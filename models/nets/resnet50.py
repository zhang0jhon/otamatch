...
class ResNet50(nn.Module):
    ...
    def __init__(
        ...
        if feat_size > 0:
            self.project_head = nn.Linear(512 * block.expansion, feat_size)
    ...
    def _forward_impl(self, x):
        ...
        elif self.feat_size > 0:
            embedding = self.project_head(x)
            return out, F.normalize(embedding, dim=1)
        else:
            return out
...
class build_ResNet50:
    def __init__(self, is_remix=False, feat_size=0):
        self.is_remix = is_remix
        self.feat_size = feat_size

    def build(self, num_classes):
        return ResNet50(n_class=num_classes, is_remix=self.is_remix, feat_size=self.feat_size)
