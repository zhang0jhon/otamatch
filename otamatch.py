...
from models.otamatch.otamatch import OTAMatch
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # SET otamatch: class otamatch in models.otamatch
    args.bn_momentum = 1.0 - 0.999
    if 'imagenet' in args.dataset.lower():
        _net_builder = net_builder('ResNet50', False, None, is_remix=False, feat_size=args.feat_size)
    else:
    ...
    ## set SGD and cosine lr on otamatch
    ...
    if args.dataset.lower() != "imagenet":
        train_dset = SSL_Dataset(args, alg='otamatch', name=args.dataset, train=True,
                                num_classes=args.num_classes, data_dir=args.data_dir)
        lb_dset, ulb_dset = train_dset.get_ssl_dset(args.num_labels)
        _eval_dset = SSL_Dataset(args, alg='otamatch', name=args.dataset, train=False,
                                num_classes=args.num_classes, data_dir=args.data_dir)
    ...
    loader_dict['train_ulb'] = get_data_loader(dset_dict['train_ulb'],
                                               ...
                                               data_sampler=args.train_sampler,
                                               num_iters=args.num_train_iter,
                                               num_workers=args.num_workers if 'imagenet' in args.dataset.lower() else 4 * args.num_workers,
                                               distributed=args.distributed)
    ...
    ## set DataLoader and ulb_dset on OTAMatch
    model.set_data_loader(loader_dict)

    model.set_dset(ulb_dset)
    
    if args.pretrain_path:
        model.load_pretrain(args.pretrain_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-sn', '--save_name', type=str, default='otamatch')
    ...
    '''
    Training Configuration of otamatch
    '''
    parser.add_argument('--temp', type=float, default=0.07)
    parser.add_argument('--feat_size', type=int, default=128)
    parser.add_argument('--tau', type=float, default=0.05)
    parser.add_argument('--epsilon_0', type=float, default=0.5)
